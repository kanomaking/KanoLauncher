...
**Window placement (off-screen fix):** the undecorated window now anchors **near the top** of the screen (title bar a fixed 28px below the work-area top, never vertically centered where the top edge could land above the screen), clamps its size to the screen, re-clamps after show, and offers **Ctrl+Shift+R to recenter** as a rescue if it ever ends up off-screen.
