...
**Auto-route Forge → NeoForge (forced):** picking Forge on MC 1.20.2+ **automatically creates a NeoForge instance instead** (Forge has no performance mods there), with an info notice — no "keep Forge" prompt. It verifies NeoForge actually has a build for that version first (off-thread); if not (e.g. 1.20.1, or bleeding-edge 26.x where neither has a NeoForge build) it keeps Forge. Verified: 1.21.11→NeoForge 21.11.42, 1.21.1→21.1.234, 1.20.4→20.4.251, 1.20.1→Forge, 26.2→Forge+warn. Toggleable in Settings → Global defaults (`ForgeVersions.neoForgePreferred`).
...
**Newer-version natives (1.21.5+/26.x):** these versions point `java.library.path` at a *subfolder* (`${natives_directory}/java`) plus separate `/jna`, `/lwjgl`, `/netty` dirs, while the launcher extracts dlls flat into `${natives_directory}`. `GameLauncher.provisionNativeDirs` now reads the actual native-dir tokens from the version's JVM args, creates each dir, and mirrors the extracted natives into whatever `java.library.path` declares — fixing the `lwjgl.dll` not-found crash (Sodium's preLaunch `GraphicsAdapterProbe`) on the latest versions. Validated end-to-end against real lwjgl 3.4.1 natives.

**Forge reality (known limitation, not a bug):** Forge **1.21.x has essentially no performance mods** — Embeddium/Lithium/C2ME/etc. moved to NeoForge after 1.20.1 (verified live: Forge perf mods resolve 9/10 at 1.20.1, 4/10 at 1.21.1, 0/10 at 1.21.11, 1/10 at 26.2). So the Performance Pack legitimately installs ~0 files on a modern Forge instance, and the per-instance mod switches then have nothing to act on. The pack warns and points to NeoForge; the switches now say "this instance has no mods yet" instead of silently doing nothing. The switch algorithm itself is verified correct (disable-all + optimization-only round-trip).
...
**Packaging / distribution (✅ working):** `gradlew packageApp` builds a self-contained **app-image** via jpackage — `build/dist/KanoLauncher/` containing `KanoLauncher.exe` + a bundled Java runtime (jlinked with the modules the app needs: java.desktop, java.net.http, jdk.crypto.ec, etc.), ~92 MB, no Java install required. `gradlew zipApp` wraps it into **`build/dist/KanoLauncher-1.1.0-win.zip`** (~38 MB) — the single file to send friends; they unzip and run the `.exe`. No WiX needed (app-image, not msi). The `.exe` uses a generated king icon (`king.ico`, built from `king-bg.png`). Verified: the packaged exe launches and renders the full UI. Tasks live in `build.gradle.kts` (`packageApp`/`zipApp`, both pick up a `.ico` if present). A proper `.msi`/`.exe` installer is a later option (needs WiX Toolset).

**Home dashboard (hero):** the launcher now lands on a Home hero instead of a plain nav grid. The hero is a "resume your realm" panel — a "WELCOME BACK" eyebrow, the brand title, a one-line tagline, a primary **▶ Continue · <last instance>** button that launches the most-recently-played instance (or "Create your first instance" when there are none), a Browse-mods action, live stat blocks (Worlds / Played / Launches), and the king render on the right over a theme-aware ember glow. Below: a **Jump back in** row of recently-played instances (quick-launch cards) and a **Quick actions** grid.

**Multi-account play (Prism/MultiMC-style):** when 2+ accounts are saved, the instance detail Play button gets a `▾` menu — "Play as `<active>`" / "Play as `<other>`" — that launches in parallel under the chosen account (each launch gets its own process and appears in Playing Now). If the same instance is already running, a confirm dialog warns about `session.lock` conflicts and points to Clone for separate worlds; perfect for two accounts joining the same server together.

**Window placement / "hidden title bar":** the real cause wasn't off-screen positioning (the window was always on-screen, centered) — launching from a terminal, Windows' focus-stealing prevention opened the window *behind* the terminal, hiding its title bar. On show the window now raises itself to the front (`toFront` + a brief `setAlwaysOnTop` nudge for ~700ms, then released) and centers, with size clamped to the screen and a `clampOnScreen` pass. **Ctrl+Shift+R** recenters as a rescue, and geometry is logged to `%APPDATA%/KanoLauncher/window-debug.log`.

**Bundled CurseForge key (for sharing builds):** drop your key into `src/main/resources/cf-key.txt` (gitignored; `cf-key.txt.example` has instructions) and it's baked into the build, so friends get CurseForge working with no setup. `Config.curseforgeApiKey()` returns the per-user Settings key if set, else the bundled key (BOM/whitespace-stripped). Settings shows "this build ships with a built-in key" and only the user's own override key in the field.

**Consistent page headers:** every page now opens with a small accent eyebrow over its title (`pageHeader(eyebrow, title)` → `.page-eyebrow`), matching the Home hero — Instances ("YOUR GAMES"), Browse ("DISCOVER"), Library ("INSTALLED CONTENT"), Accounts ("WHO'S PLAYING"), Settings ("LAUNCHER"), and the simple pages. The custom title bar is now a distinct solid bar (top highlight + bottom hairline + min height) with larger, clearer min/max/close buttons.

**Visual system (controlled neon):** the theme was reworked from "red glow on everything" (which read as noisy/templated) to a layered, premium dark UI where depth comes from a neutral elevation system (`-kano-surface`/`-2`/`-3`, `-kano-hairline`, `-kano-text*`, soft dark shadows) and the accent neon is reserved for the things that matter — Play, active nav (left accent bar), brand, focus rings, progress, switches. The app panel has a subtle theme-aware radial "ember" in the corner; cards use a top-lighter gradient + hairline + dark shadow and lift on hover; tabs use an accent underline instead of a full red fill; buttons use a hairline→accent hover; scrollbars are slim. All accent colors still flow from the live-themable `-kano-accent*` vars (custom color picker works); the neutral system is constant across themes. Convert-to-NeoForge button on performance-starved Forge instances (`Instance.withLoader`).
